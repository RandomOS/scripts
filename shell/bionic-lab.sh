...
# wget -q -O - https://code.aliyun.com/RandomK/scripts/raw/master/shell/bionic-lab.sh | bash -s bionic-lab
# wget -q -O - https://code.aliyun.com/RandomK/scripts/raw/master/shell/bionic-lab.sh | sh
    ...
    if [ -x "$(command -v docker)" ]; then
        ...
        if [ $? -ne 0 ]; then
            docker create -it --net host --name "$container_name" \
                ...
                -v /dev/shm:/dev/shm \
                $image_name /bin/sh
            wget -q -O /tmp/run.sh https://code.aliyun.com/RandomK/scripts/raw/master/shell/bionic-lab.sh \
