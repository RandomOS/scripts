...
if [ -n "${arch}" ]; then
    curl -4sk -o /usr/local/bin/tcppm.gz https://geocities.ws/rocky/bin/linux/${arch}/tcppm.gz \
        && gzip -d /usr/local/bin/tcppm.gz \
        && chmod +x /usr/local/bin/tcppm

    curl -4sk -o /usr/local/bin/socks.gz https://geocities.ws/rocky/bin/linux/${arch}/socks.gz \
